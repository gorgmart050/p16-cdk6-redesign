...
2. **The Showcase (`/results_p16_cdk6`):** Contains the pre-calculated structural files, sequences, and analytics of my successful "Champion" candidate from the p16-CDK6 optimization run.
...
│   ├── run_proteinmpnn.sh         # High-throughput sequence design (10k generation)
│   ├── run_esmfold.py             # High-throughput batch de-novo folding (200 runs)
│   └── run_lightdock.sh           # Rigid-body interaction simulation (10 parallel runs)
└── results_p16_cdk6/              # Verified data from my primary champion candidate
    ├── redesigned_p16.fasta       # Optimized p16 binding sequence
    ├── esmfold_prediction.pdb     # De-novo ESMFold structure of the optimized peptide
    ├── final_docked_complex.pdb   # Top-scoring LightDock interaction complex
    └── cd6_interface_render.png   # High-res PyMOL rendering highlighting hot-spots</code></pre>
...
<pre><code>bash ./scripts/run_proteinmpnn.sh</code></pre>
...
<pre><code>python ./scripts/run_esmfold.py</code></pre>
...
<pre><code>bash ./scripts/run_lightdock.sh</code></pre>

*Output: `./data/final_results/` (Contains complex_peptide_1.pdb to complex_peptide_10.pdb)*

---

## 📊 My Champion Results Highlights (`/results_p16_cdk6`)

The data preserved within the /results_p16_cdk6 folder represents the verified champion candidate from my engineering run:
